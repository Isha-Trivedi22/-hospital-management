...
public class AppointmentPanel {
    ...
    public VBox getView() {
        ...
        dateField.setPromptText("Date (2026-05-25)");
        timeField.setPromptText("Time (10:30 AM)");
        statusField.setPromptText("Status");

        Button addBtn     = styledBtn("➕ Book", "#1a73e8");
        Button deleteBtn  = styledBtn("🗑 Cancel", "#e53935");
        Button refreshBtn = styledBtn("🔄 Refresh", "#43a047");

        addBtn.setOnAction(e -> {
            String json = String.format(
                "{\"patientName\":\"%s\",\"doctorName\":\"%s\",\"date\":\"%s\",\"time\":\"%s\",\"status\":\"%s\"}",
                patientField.getText(), doctorField.getText(),
                dateField.getText(), timeField.getText(), statusField.getText());
            ApiClient.post("/appointments", json);
            patientField.clear(); doctorField.clear(); dateField.clear();
            timeField.clear(); statusField.clear();
            loadData();
        });

        deleteBtn.setOnAction(e -> {
            String[] sel = table.getSelectionModel().getSelectedItem();
            if (sel != null) { ApiClient.delete("/appointments/" + sel[0]); loadData(); }
        });

        refreshBtn.setOnAction(e -> loadData());

        HBox form = new HBox(8, patientField, doctorField, dateField,
                             timeField, statusField, addBtn, deleteBtn, refreshBtn);
    ...
    private void loadData() {
        ...
        try {
            JsonArray arr = JsonParser.parseString(ApiClient.get("/appointments")).getAsJsonArray();
            for (JsonElement el : arr) {
                ...
                table.getItems().add(new String[]{
                    ...
                    o.get("time").getAsString(), o.get("status").getAsString()
                });
            }
